...
function Canvas(host, options) {
  ...
  this.userAgent = options.userAgent || options['user-agent'] || 'canvas-lms-api';
}
...
Canvas.prototype._http = function (options, prevBody) {
  ...
  return delay(throttle)
    ...
    .then(([response, body]) => {
      ...
      if (isArrayBody && response.headers.get("link")) {
        ...
        if (nextLink && nextLink.next) {
          ...
          const merged = prevBody ? union(prevBody, body) : body;
          return that._http(nextOptions, merged);
        }
      }
      if (prevBody) {
        if (Array.isArray(prevBody) && Array.isArray(body)) {
...
Canvas.prototype.graphql = function (query, variables = {}) {
  const url = resolve(this.host, '/api/graphql');
  const options = {
    method: 'POST',
    url,
    headers: {
